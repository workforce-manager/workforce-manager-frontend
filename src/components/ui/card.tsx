...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-6 rounded-sm shadow-sm text-white bg-[#b5a7f014] border border-transparent transition-all duration-300 ease-in-out",
        className
      )}
      {...props}
    />
...
function CardContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("px-0 text-[16px]", className)}
      {...props}
    />
  )
}
